...
Proof.
  ...
  (* T_m(n.+2) = n.+2 ^ m - n.+2 + (n.+3 ^ m - 1 - n.+2 ^m) *)
  rewrite -IHn.
  (* T_m(n.+2) = T_m(n.+1) + (n.+3 ^ m - 1 - n.+2 ^m) *)
  rewrite [in n.+3](_ : n.+3 = 1+n.+2) //.
  rewrite Pascal.
  (* T_m(n.+2) = T_m(n.+1) + (\sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i) - 1 - n.+2 ^m) *)
  transitivity (Tm n.+1 + (\sum_(1 <= k < m) 'C(m,k) * n.+2^k)).
    (* T_m(n.+2) = T_m(n.+1) + \sum_{k=1}^{m-1} \binom{m}{k} n.+2^k *)
    rewrite -big_split /=.
    apply eq_bigr => i _.
    rewrite /Sk.
    ...
    rewrite big_nat1.
    by rewrite mulnDr.
  (* T_m(n.+1) + \sum_{k=1}^{m-1} \binom{m}{k} n.+2^k = T_m(n.+1) + \sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i - 1 - n.+2 ^m *)
  congr (_ + _).
  (* \sum_{k=1}^{m-1} \binom{m}{k} n.+2^k = \sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i - 1 - n.+2 ^m *)
  transitivity ((\sum_(0 <= k < m.+1) 'C(m,k) * n.+2^k) - 1 - n.+2^m).
    (* \sum_{k=1}^{m-1} \binom{m}{k} n.+2^k = \sum_{k=0}^m \binom{m}{k} n.+2^k - 1 - n.+2^m *)
    symmetry.
    rewrite (@big_cat_nat _ _ _ 1) //=.
    rewrite addnC big_nat1 bin0 expn0 mul1n addnK.
    rewrite (@big_cat_nat _ _ _ m) //=.
    by rewrite big_nat1 binn mul1n addnK.
  (* \sum_{k=0}^m \binom{m}{k} n.+2^k - 1 - n.+2^m = \sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i - 1 - n.+2 ^m *)
  rewrite big_mkord.
  (* \sum_{i<m+1}^m \binom{m}{i} n.+2^i - 1 - n.+2^m = \sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i - 1 - n.+2 ^m *)
  do 2 congr (_ - _).
  (* \sum_{i<m+1}^m \binom{m}{i} n.+2^i = \sum_{i<m+1} \binom{m}{i} 1^{m-i} n.+2 ^ i *)
  apply eq_bigr => i _.
  (* \binom{m}{i} n.+2^i = \binom{m}{i} 1^{m-i} n.+2 ^ i *)
  by rewrite exp1n mul1n.
Qed.
